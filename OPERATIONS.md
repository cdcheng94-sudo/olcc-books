...
- 之前没传的行 → 显示 **补传凭证** 按钮,随时补。**也可以直接把文件从桌面拖到那一行的「凭证」格子上**,松手就传(格子会亮金色)。表单里的凭证栏位同样支持拖曳。
...
发过的收据客户名下面会出现绿色 **「已发送 日期」**(发过不止一次会显示次数),✉ 图标也变绿。**还是可以再发** —— 只是按下去会先提醒你上次是几时发的,避免重复寄。
...
### "发票开错了 / 客户改方案了,旧单怎么办"
**不要删,点 🚫 作废。** 那行右边的 🚫 → 写原因(例如「改成年缴,并入 INV-0007」)→ 确认。
- 编号保留、状态变「已作废」、原因显示在编号下面 —— 以后翻账看得懂为什么这张没收钱。
- 已付的单不能作废(它已经生成收据和收入了);真要撤销找开发员。
- 如果作废的是订阅自动开的那期,系统会允许你对同一期重开一张正确的。
